...
> = ({ block, isSelected, onUpdate, onDuplicate, onDelete, blockIndex }) => {
  ...
  const handleDuplicateTitle = (cardId: string, titleContent: string) => {
    const updatedCards = block.cards.map((card) => {
      if (card.id === cardId) {
  ...
  const handleDuplicateDescription = (
    cardId: string,
    descContent: string,
  ) => {
    const updatedCards = block.cards.map((card) => {
  ...
  const handleDeleteTitleField = (cardId: string, titleId: string) => {
    const updatedCards = block.cards.map((card) => {
      if (card.id === cardId) {
        return {
          ...card,
          titles: (card.titles || []).filter((t) => t.id !== titleId),
        };
      }
      return card;
    });
  ...
  const handleDeleteDescriptionField = (
    ...
    const updatedCards = block.cards.map((card) => {
      if (card.id === cardId) {
        return {
          ...card,
          descriptions: (card.descriptions || []).filter(
            (d) => d.id !== descId,
          ),
        };
      }
          ...
          >
            ...
            >
              ...
              {card.titles && card.titles.length > 0 && (
                <div className="mt-3">
                  {card.titles.map((titleField) => (
                    <div
                      key={titleField.id}
                      onMouseEnter={() =>
                        setHoveredField(`${card.id}-title-${titleField.id}`)
                      }
                      onMouseLeave={() => setHoveredField(null)}
                    >
                      <p
                        ...
                        style={{
                          color: card.textColor,
                          border:
                            focusedField === `${card.id}-title-${titleField.id}`
                              ? "2px solid rgb(255, 106, 0)"
                              : hoveredField ===
                                  `${card.id}-title-${titleField.id}`
                                ? "2px dotted rgb(255, 106, 0)"
                                : "none",
                        }}
                        onClick={() =>
              ...
              {card.descriptions && card.descriptions.length > 0 && (
                <div className="mt-3">
                  {card.descriptions.map((descField) => (
                    <div
                      key={descField.id}
                      onMouseEnter={() =>
                        setHoveredField(
                          `${card.id}-description-${descField.id}`,
                        )
                      }
                      onMouseLeave={() => setHoveredField(null)}
                    >
